      ...
      {isAnalyzing && (
        <div className="text-center py-12 space-y-4">
          ...
          <p className="text-xs text-slate-400 dark:text-gray-500">
            Checking LTV, PTI, and Guidelines
          </p>
        </div>
      )}
      ...
      {suggestionResult && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="space-y-4">
            {(Array.isArray(suggestionResult.suggestions) ? suggestionResult.suggestions : []).map(
              (suggestion, idx) => {
                const alternativeVehicle = suggestion.alternativeVehicleVin
                  ? findAlternativeVehicle(suggestion.alternativeVehicleVin)
                  : null;

                return (
                  <div
                    key={idx}
                    className="border border-slate-200 dark:border-gray-800 rounded-xl overflow-hidden bg-white dark:bg-black shadow-sm hover:shadow-md transition-all duration-200"
                  >
                    ...
                    {alternativeVehicle ? (
                      <div className="p-4 bg-green-50/50 dark:bg-green-900/10 text-sm">
                        <div className="flex justify-between items-center mb-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-gray-400">
                              Stock: {alternativeVehicle.stock} | Price:{" "}
                              {formatCurrency(alternativeVehicle.price)}
                            </p>
                          </div>
                          <div className="text-right">
                            ...
                            <p className="font-bold text-green-600 dark:text-green-400 text-lg">
                              {formatCurrency(alternativeVehicle.frontEndGross)}
                            </p>
                          </div>
                        </div>
                        <div className="flex justify-end">
                          <Button
                            size="sm"
                            onClick={() => handleSwitchInventory(alternativeVehicle.vin)}
                      ...
                      <div className="p-4 bg-white dark:bg-black text-sm">
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          typeof suggestion.proposedChanges === "object" ? (
                            ...
                            <span className="text-slate-500 dark:text-gray-400">
                              No structured changes provided.
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <Button
            variant="ghost"
            onClick={handleAnalyze}
            className="w-full text-xs mt-4 text-slate-500 hover:text-slate-800 dark:text-gray-500 dark:hover:text-gray-300"
          >
