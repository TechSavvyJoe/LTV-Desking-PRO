...
const ToolContentWrapper: React.FC<{
  title: string;
  children: React.ReactNode;
}> = ({ title, children }) => (
  <div>
...
const ToolInventorySearch = ({
  ...
  onSelect: (vehicle: CalculatedVehicle) => void;
  favorites: Vehicle[];
        ...
        <div className="absolute z-10 w-full mt-1 bg-slate-50 dark:bg-black border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg">
          <ul className="py-1">
            {filteredInventory.map((v) => (
              <li
                key={v.vin}
                className="flex items-center justify-between px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
                onClick={() => {
                  onSelect(v);
                  setQuery("");
                }}
              >
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {v.stock} - {formatCurrency(v.price)}
                  </p>
                </div>
                <button
                  onClick={(e) => {
                ...
                >
                  <Icons.StarIcon
                    className={`transition-colors ${
                      favoriteVins.has(v.vin)
                        ? "text-yellow-500 fill-yellow-500"
                        : "text-slate-400 dark:text-slate-300 group-hover:text-yellow-500"
                    }`}
                  />
                </button>
              </li>
      ...
      <div className="space-y-6">
              ...
              <div key={i} className="grid grid-cols-10 gap-2 items-center">
                ...
                <div className="col-span-2 flex items-center gap-2">
                  <span className="text-sm font-medium text-green-600 dark:text-green-400">
                    {formatCurrency(Number(p.price) - Number(p.cost))}
                  </span>
                  <Button
                    size="sm"
                    variant="danger"
        ...
        <div className="pt-4 mt-4 border-t border-slate-200 dark:border-x-border space-y-2">
          <ResultDisplay
            ...
            value={formatCurrency(frontEndGross)}
            valueColorClass={
              frontEndGross > 0
          ...
          <ResultDisplay
            ...
            value={formatCurrency(backEndGross)}
            valueColorClass={
              backEndGross > 0
                ? "text-green-600 dark:text-green-400"
                : "text-slate-500 dark:text-slate-400"
...
const DealComparison = ({
  ...
  if (!activeVehicle)
    return (
      <ToolContentWrapper title="Deal Comparison">
        <p className="text-center text-slate-500 dark:text-slate-400">
          Select a vehicle from the inventory to start comparing deals.
        </p>
      </ToolContentWrapper>
    );
  ...
  const DealColumn = ({
    ...
    <div className="space-y-2 p-3 bg-slate-100 dark:bg-slate-900 rounded-lg">
      ...
      <InputGroup label="Backend">
        <StyledInput
          name="backendProducts"
          type="number"
          value={deal.backendProducts}
          onChange={handler}
        />
      </InputGroup>
      <div className="pt-2 mt-2 border-t border-slate-200 dark:border-x-border text-center space-y-2">
        <div className="font-bold text-2xl text-x-blue">
          {formatCurrency(calculated?.monthlyPayment)}/mo
        </div>
  ...
  return (
    <ToolContentWrapper title={`Comparing Deals for ${activeVehicle.vehicle}`}>
      <div className="grid grid-cols-2 gap-4">
        <DealColumn
          title="Deal A"
          deal={dealA}
          handler={handleAChange}
          calculated={calculatedA}
          onCopy={() => copyToGlobal(dealA)}
        />
        <DealColumn
          title="Deal B"
          deal={dealB}
          handler={handleBChange}
          calculated={calculatedB}
          onCopy={() => copyToGlobal(dealB)}
        />
      </div>
    </ToolContentWrapper>
  );
...
const ReserveCalculator = () => {
  ...
  const { totalReserve, dealerShare } = useMemo(() => {
    ...
    const paymentAtSell = calculateMonthlyPayment(
      principal,
      Number(sellRate),
      t
    );
    ...
    if (typeof paymentAtSell !== "number" || typeof paymentAtBuy !== "number")
      return { totalReserve: 0, dealerShare: 0 };
  ...
  return (
    <ToolContentWrapper title="Reserve Calculator">
      <div className="space-y-4">
        <InputGroup label="Amount Financed ($)">
          <StyledInput
            type="number"
            value={amountFinanced}
            onChange={(e) => setAmountFinanced(Number(e.target.value) || "")}
          />
        </InputGroup>
        <div className="grid grid-cols-2 gap-4">
          <InputGroup label="Buy Rate (APR %)">
            <StyledInput
              type="number"
              step="0.01"
              value={buyRate}
          ...
          <InputGroup label="Sell Rate (APR %)">
            <StyledInput
              type="number"
              step="0.01"
              value={sellRate}
        ...
        <div className="pt-4 mt-4 border-t border-slate-200 dark:border-x-border space-y-2">
          ...
          <ResultDisplay
            label="Dealer Share"
            value={formatCurrency(dealerShare)}
...
const FlatCalculator = () => {
  ...
  const fee = useMemo(
    () => Number(amountFinanced) * (Number(flatPercent) / 100),
    [amountFinanced, flatPercent]
  );

  return (
    <ToolContentWrapper title="Flat Fee Calculator">
      <div className="space-y-4">
        <InputGroup label="Amount Financed ($)">
          <StyledInput
            type="number"
...
const TermComparator = () => {
  ...
  return (
    <ToolContentWrapper title="Term vs. Payment Comparator">
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <InputGroup label="Interest Rate (APR %)">
            <StyledInput
              type="number"
              step="0.1"
              value={interestRate}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700">
                <th className="p-2 font-semibold text-slate-500 dark:text-slate-300">
                  Term
                </th>
                <th className="p-2 font-semibold text-right text-slate-500 dark:text-slate-300">
                  Monthly Payment
                </th>
              </tr>
            </thead>
            <tbody>
              {payments.map((p) => (
                <tr
                  key={p.term}
                  className="border-b border-slate-200 dark:border-x-border last:border-0"
                >
                  <td className="p-2">{p.term} mo</td>
                  <td className="p-2 text-right font-semibold text-x-blue">
...
const QuoteBuilder = ({
  ...
  const { amountToFinance, monthlyPayment } = useMemo(() => {
    ...
    const taxRate = 0.06;
    const taxableAmount =
      Math.max(0, price - trade) + settings.docFee + settings.cvrFee;
    const salesTax = taxableAmount * taxRate;

    const otd =
      price + settings.docFee + settings.cvrFee + stateFees + salesTax;
  ...
  return (
    <ToolContentWrapper title="Quote Builder Sandbox">
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <InputGroup label="Sale Price ($)">
            <StyledInput
          ...
          <InputGroup label="Down Payment ($)">
            <StyledInput
              name="downPayment"
              type="number"
              value={localDeal.downPayment}
              onChange={handleChange}
            />
          </InputGroup>
          <InputGroup label="Trade Value ($)">
            <StyledInput
              name="tradeInValue"
              type="number"
              value={localDeal.tradeInValue}
              onChange={handleChange}
            />
          </InputGroup>
          <InputGroup label="Trade Payoff ($)">
            <StyledInput
              name="tradeInPayoff"
              type="number"
              value={localDeal.tradeInPayoff}
              onChange={handleChange}
            />
          </InputGroup>
          <InputGroup label="Backend ($)">
            <StyledInput
              name="backendProducts"
              type="number"
              value={localDeal.backendProducts}
              onChange={handleChange}
            />
          </InputGroup>
          <InputGroup label="State Fees ($)">
            <StyledInput
              name="stateFees"
              type="number"
              value={localDeal.stateFees}
              onChange={handleChange}
            />
          </InputGroup>
          <InputGroup label="Term">
...
const TradeEquityCalculator = () => {
  ...
  const netEquity = useMemo(
    () => Number(tradeValue) - Number(tradePayoff),
    [tradeValue, tradePayoff]
  );
  const colorClass =
    netEquity >= 0 ? "text-green-600 dark:text-green-400" : "text-red-500";
  return (
    <ToolContentWrapper title="Trade Equity Calculator">
      <div className="space-y-4">
        <InputGroup label="Trade-In Value ($)">
          <StyledInput
            type="number"
            value={tradeValue}
            onChange={(e) =>
              setTradeValue(e.target.value === "" ? "" : Number(e.target.value))
            }
            min="0"
            placeholder="e.g., 15000"
          />
        </InputGroup>
        <InputGroup label="Trade-In Payoff ($)">
          <StyledInput
            type="number"
            value={tradePayoff}
            onChange={(e) =>
...
const PaymentCalculator = () => {
  ...
  return (
    <ToolContentWrapper title="Payment Calculator">
      <div className="space-y-4">
        <InputGroup label="Loan Amount ($)">
          <StyledInput
            type="number"
        ...
        <InputGroup label="Loan Term (Months)">
          <StyledSelect
            value={loanTerm}
...
const AffordabilityCalculator = () => {
  ...
  const maxLoan = useMemo(
    ...
    [monthlyPayment, interestRate, loanTerm]
  );
  return (
    <ToolContentWrapper title="Affordability Calculator">
      <div className="space-y-4">
        <InputGroup label="Desired Monthly Payment ($)">
          <StyledInput
            type="number"
...
const LenderReference = ({ lenderProfiles }: ToolProps) => {
  ...
  return (
    <ToolContentWrapper title="Lender Quick Reference">
      <StyledInput
      ...
      <div className="mt-4 space-y-2 max-h-[70vh] overflow-y-auto">
        {filteredProfiles.map((p) => (
          ...
          >
            ...
            {expanded === p.id && (
              <div className="p-3 border-t border-slate-200 dark:border-x-border text-xs space-y-1">
...
const tools: {
...
}[] = [
  {
    ...
    component: AiDealAssistant,
  },
  {
    id: "eligibility",
    name: "Lender Match",
    icon: <Icons.ShieldCheckIcon />,
    component: LenderEligibility,
  },
  {
    id: "gross",
    name: "Gross Profit",
    ...
    >
      ...
      >
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="font-extrabold text-sm tracking-tight [writing-mode:vertical-rl] rotate-180">
            Toolbox
          </span>
        </div>
      </div>
