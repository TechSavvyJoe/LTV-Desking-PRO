...
const InventoryTable: React.FC<InventoryTableProps> = ({ 
  ...
  const columns = [
    ...
    { header: 'Actions', accessor: 'action' as const, className: 'min-w-[220px]', render: (item: CalculatedVehicle) => {
        if (!item) return null;
        return (
          <div className="flex items-center gap-2">
            <button
              onClick={(e) => { e.stopPropagation(); toggleFavorite(item.vin); }}
              className={`p-2.5 rounded-full transition-colors group focus:outline-none`}
              aria-label="Toggle Favorite"
            >
              <Icons.StarIcon className={`transition-colors ${favoriteVins.has(item.vin) ? 'text-yellow-500 fill-yellow-500' : 'text-x-text-secondary group-hover:text-yellow-500'}`} />
            </button>
            <Button
              variant="secondary"
              size="sm"
    ...
    { header: 'Price', accessor: 'price' as const, isNumeric: true, className: 'text-right', render: (item: CalculatedVehicle) => <CopyToClipboard valueToCopy={item.price}>{formatCurrency(item.price)}</CopyToClipboard> },
    ...
    { header: 'VIN', accessor: 'vin' as const, render: (item: CalculatedVehicle) => <CopyToClipboard valueToCopy={item.vin}>{item.vin}</CopyToClipboard> },
  ];
  ...
  const renderExpandedRow = (item: CalculatedVehicle) => {
    ...
    try {
        ...
        console.error("Error checking eligibility in expanded row:", e);
    }
            ...
            <div className="text-sm space-y-1.5 max-h-48 overflow-y-auto pr-2">
              {eligibilityDetails.map(detail => (
                <div key={detail.name} className="flex justify-between items-start text-sm gap-4">
                  <span className="font-medium text-x-text-secondary whitespace-nowrap">{detail.name}</span>
                  {detail.eligible ? (
  ...
  return (
    <div className="my-8">
       <h2 className="text-xl font-bold text-x-text-primary mb-4 pb-2 border-b-2 border-x-border">Inventory</h2>
      <Table 
        columns={columns} 
        data={safeVehicles} 
        sortConfig={sortConfig} 
