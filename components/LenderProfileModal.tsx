...
const LenderProfileModal: React.FC<LenderProfileModalProps> = ({ profile, isOpen, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setFormData({ id: '', name: '', ...NEW_PROFILE_TEMPLATE });
    }
  }, [profile, isOpen]);
  ...
  const handleGeneralChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => ({ ...prev, [name]: isNumber ? (value ? Number(value) : 0) : value }));
  };
  ...
  const addTier = () => {
      ...
      const lastTier = currentTiers.length > 0 ? currentTiers[currentTiers.length - 1] : {};
      const newTier = { ...lastTier, name: `Tier ${(currentTiers.length || 0) + 1}`};
  ...
  const removeTier = (index: number) => {
      ...
      setFormData(prev => ({...prev, tiers: currentTiers.filter((_, i) => i !== index)}));
  };
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-4 mb-6">
                 <div className="sm:col-span-2 lg:col-span-1">
                    <InputGroup label="Lender Name">
                        <StyledInput type="text" name="name" value={formData.name} onChange={handleGeneralChange} required />
                    </InputGroup>
                 </div>
                 <InputGroup label="Book Value Source">
                    <StyledSelect name="bookValueSource" value={formData.bookValueSource || 'Trade'} onChange={handleGeneralChange}>
                        <option value="Trade">JD Power Trade</option>
                        <option value="Retail">JD Power Retail</option>
                    </StyledSelect>
                 </InputGroup>
                 <InputGroup label="Min Monthly Income ($)">
                    <StyledInput type="number" name="minIncome" value={formData.minIncome || ''} onChange={handleGeneralChange} min="0" />
                 </InputGroup>
                 <InputGroup label="Max PTI (%)">
                    <StyledInput type="number" name="maxPti" value={formData.maxPti || ''} onChange={handleGeneralChange} min="0" />
                 </InputGroup>
            </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                       <div className="col-span-2 md:col-span-4 lg:col-span-2">
                          <InputGroup label="Tier Name">
                              <StyledInput type="text" name="name" value={tier.name} onChange={e => handleTierChange(index, e)} required/>
                          </InputGroup>
                       </div>
                       <InputGroup label="Min FICO">
                           <StyledInput type="number" name="minFico" value={tier.minFico ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Max FICO">
                           <StyledInput type="number" name="maxFico" value={tier.maxFico ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Min Year">
                           <StyledInput type="number" name="minYear" value={tier.minYear ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Max Year">
                           <StyledInput type="number" name="maxYear" value={tier.maxYear ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Min Mileage">
                           <StyledInput type="number" name="minMileage" value={tier.minMileage ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Max Mileage">
                           <StyledInput type="number" name="maxMileage" value={tier.maxMileage ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Min Term (mo)">
                           <StyledInput type="number" name="minTerm" value={tier.minTerm ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Max Term (mo)">
                           <StyledInput type="number" name="maxTerm" value={tier.maxTerm ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Max LTV (%)">
                           <StyledInput type="number" name="maxLtv" value={tier.maxLtv ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Min Fin. ($)">
                           <StyledInput type="number" name="minAmountFinanced" value={tier.minAmountFinanced ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                       <InputGroup label="Max Fin. ($)">
                           <StyledInput type="number" name="maxAmountFinanced" value={tier.maxAmountFinanced ?? ''} onChange={e => handleTierChange(index, e)} />
                       </InputGroup>
                    </div>
                     <Button type="button" variant="danger" size="sm" className="!p-1.5 !rounded-full absolute top-2 right-2" onClick={() => removeTier(index)}>&times;</Button>
                </div>
            ))}
            </div>
